...
type ty =
  ...
  | TFuture of ty (** future values [future ty] *)
...
type expr =
  ...
  | Async of expr (** async computation [async e] *)
  | Future of int (** future value (internal) *)
...
let string_of_type ty =
  let rec to_str n ty =
	...
	| TFuture ty -> (3, "future " ^ (to_str 3 ty))
    in
...
let rec string_of_value = function
  ...
  | Record rs ->
	...
	"}"
  | Closure _ -> "<fun>"
  | Future id -> "<future#" ^ string_of_int id ^ ">"
  | _ -> assert false
